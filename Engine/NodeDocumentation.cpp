...
Node::makeDocumentation(bool genHTML) const
{
...

    {
        ...
        for (int i = 0; i < _imp->effect->getNInputs(); ++i) {
            ...
            input << ( _imp->effect->isInputOptional(i) ? tr("Да") : tr("нет") );
    ...
    for (KnobsVec::const_iterator it = knobs.begin(); it != knobs.end(); ++it) {
        ...
        if (isInt) {
            knobType = tr("Целое число");
        } else if (isChoice) {
            knobType = tr("Выбор");
        } else if (isBool) {
            knobType = tr("Логический");
        } else if (isDbl) {
            knobType = tr("Двойной");
        } else if (isString) {
            if (isLabel) {
            ...
            knobType = tr("Сепаратор");
        } else if (isBtn) {
            knobType = tr("Кнопка");
        } else if (isParametric) {
            knobType = tr("Параметрический");
        } else if (isGroup) {
            knobType = tr("Группа");
        } else if (isPage) {
            knobType = tr("Страница");
        } else if (isColor) {
            knobType = tr("Цвет");
        } else {
            knobType = tr("Н/Д");
            ...
            for (int i = 0; i < (*it)->getDimension(); ++i) {
                ...
                if (!isBtn && !isSep && !isParametric) {
                    ...
                    if ( isChoice &&
                        ...
                        valueStr = isBool->getDefaultValue(i) ? tr("Вкл") : tr("Выкл");
                    } else if (isString) {
    ...
    if ( pluginID.startsWith( QString::fromUtf8("net.sf.openfx.Merge") ) ) {
        ...
        if ( !op.empty() ) {
            ...
            // but pandoc borks it
            ms << tr("Узел *%1* идентичный %2, за исключением того, что по умолчанию для оператора установлено значение *%3*")
            .arg(pluginLabel)
    ...
    ms << tr("*Документация предназначена для версии %1.%2 %3 (%4).*").arg(majorVersion).arg(minorVersion).arg(pluginLabel).arg(pluginID) << "\n\n";
    ...
    ms << "\n" << tr("Входы") << "\n--------------------------------------------------------------------------------\n\n";
    ms << tr("вход") << " | " << tr("Описание") << " | " << tr("Дополнительно") << "\n";
    ms << "--- | --- | ---\n";
    if (inputs.size() > 0) {
    ...
    ms << tr("Имя параметра / скрипта") << " | " << tr("Тип") << " | " << tr("По умолчанию") << " | " << tr("Функция") << "\n";
    ms << "--- | --- | --- | ---\n";
    if (items.size() > 0) {
