    ...
    , _imp( new TrackerPanelPrivate(this, n) )
{
    ...
    _imp->trackLabel = new ClickableLabel(tr("Ключевой кадр трека:"), trackContainer);
    ...
    _imp->currentKeyframe->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Текущий ключевой кадр шаблона для выбранных дорожек."), NATRON_NAMESPACE::WhiteSpaceNormal) );
    trackLayout->addWidget(_imp->currentKeyframe);

    _imp->ofLabel = new ClickableLabel(tr("из"), trackContainer);
    ...
    _imp->totalKeyframes->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Количество ключевых кадров шаблона для всех выбранных дорожек."), NATRON_NAMESPACE::WhiteSpaceNormal) );
    ...
    _imp->prevKeyframe->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Перейдите к предыдущему ключевому кадру шаблона."), NATRON_NAMESPACE::WhiteSpaceNormal) );
    ...
    _imp->nextKeyframe->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Перейдите к следующему ключевому кадру шаблона."), NATRON_NAMESPACE::WhiteSpaceNormal) );
    ...
    _imp->addKeyframe->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Добавьте ключевой кадр к шаблону на текущей временной шкале."), NATRON_NAMESPACE::WhiteSpaceNormal) );
    ...
    _imp->removeKeyframe->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Удалите ключевой кадр из шаблона на текущей временной шкале."), NATRON_NAMESPACE::WhiteSpaceNormal) );
    ...
    _imp->clearAnimation->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Удалите все ключевые кадры в шаблоне для выбранных дорожек."), NATRON_NAMESPACE::WhiteSpaceNormal) );
    ...
    dimensionNames << tr("Включить") << tr("Метка") << tr("Модель движения") <<
        tr("Центр X") << tr("Центр Y") << tr("Смещение X") << tr("Смещение Y") <<
        tr("Ошибка");
    ...
    _imp->addButton->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Добавить новый трек"), NATRON_NAMESPACE::WhiteSpaceNormal) );
    ...
    _imp->removeButton->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Удаление выбранных дорожек"), NATRON_NAMESPACE::WhiteSpaceNormal) );
    ...
    _imp->selectAllButton->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Выберите все треки"), NATRON_NAMESPACE::WhiteSpaceNormal) );
    ...
    _imp->resetTracksButton = new Button(tr("Сброс"), _imp->buttonsContainer);
    ...
    _imp->resetTracksButton->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Сброс выбранных дорожек"), NATRON_NAMESPACE::WhiteSpaceNormal) );

    _imp->averageTracksButton = new Button(tr("Средний"), _imp->buttonsContainer);
    ...
    _imp->averageTracksButton->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Создает новый трек в качестве среднего значения выбранных треков"), NATRON_NAMESPACE::WhiteSpaceNormal) );
                                       ...
                                       TrackDatas& data)
{
    ///Enabled
    {
        ...
        newItem->setToolTip( NATRON_NAMESPACE::convertFromPlainText(tr("Если флажок снят, этот трек больше не будет отслеживаться, даже если он установлен. Кроме того, параметры преобразования на вкладке Преобразование не будут учитывать этот трек"), NATRON_NAMESPACE::WhiteSpaceNormal) );
        d.item = newItem;
        d.dimension = -1;
