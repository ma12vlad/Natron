...
CurveWidget::pasteKeyFramesFromClipBoardToSelectedCurve()
{
    ...
    if (!curve) {
        Dialogs::warningDialog( tr("Source").toStdString(), tr("You must select a curve first.").toStdString() );
...
CurveWidget::loopSelectedCurve()
{
    ...
    if (!curve) {
        Dialogs::warningDialog( tr("Source").toStdString(), tr("You must select a curve first in the view.").toStdString() );
...
CurveWidget::frameSelectedCurve()
{
    ...
    if ( selection.empty() ) {
        frameAll();
        //Dialogs::warningDialog( tr("Source").toStdString(), tr("You must select a curve first in the left pane.").toStdString() );
    } else {
...
CurveWidget::exportCurveToAscii()
{
    ...
    for (Curves::iterator it = _imp->_curves.begin(); it != _imp->_curves.end(); ++it) {
        ...
        if ( (*it)->isVisible() && isKnobCurve ) {
            ...
            if (isString) {
                Dialogs::warningDialog( tr("Source").toStdString(), tr("String curves cannot be imported/exported.").toStdString() );
    ...
    if ( curves.empty() ) {
        Dialogs::warningDialog( tr("Source").toStdString(), tr("You must have a curve on the editor first.").toStdString() );
